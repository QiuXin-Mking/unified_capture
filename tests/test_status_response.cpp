...
int main() {
    ...
    assert(json.find("\"product\":\"mango\"") != std::string::npos);
    assert(json.find("\"product\":\"mango\"", json.find("\"product\":\"mango\"") + 1) == std::string::npos);
    ...
    const CaptureSensorStatus mango_sensors = capture_sensor_status(
        ProductProfile::mango, true, true, true);
