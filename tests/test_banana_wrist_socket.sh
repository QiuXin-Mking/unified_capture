...
assert_outputs() {
    local session_dir=$1 camera=$2 camera_dir="$session_dir/$camera"
    local mkvs=() imus=()
    shopt -s nullglob
    mkvs=("$camera_dir"/*.mkv)
...
case "$CASE" in
    two)
        ...
        ;;
    one)
        left=$(find "$session_dir/wrist_left" -type f \( -name '*.mkv' -o -name '*.jsonl' \) -print -quit 2>/dev/null || true)
        right=$(find "$session_dir/wrist_right" -type f \( -name '*.mkv' -o -name '*.jsonl' \) -print -quit 2>/dev/null || true)
