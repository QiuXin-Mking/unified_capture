...
- `mango`（双目档）：`discover_mango_cameras()` 取一块**不与 JHH04 同 bus** 的 SLZH 双目（`1bcf:2d56`）作 `head`，加左右腕共 3 路。
...
| 硬件 | 接口 | 说明 |
|------|------|------|
| 独立双目模组（SLZH） | `1bcf:2d56` | 头部双目，4000×1200（左右拼接，各 2000×1200） |
...
|------|---------|--------|------|------|----------|
| 头部双目（SLZH） | `1bcf:2d56` | 4000×1200 | 30fps | 16 Mbps | H.265 视频 + IMU 码带（加速度/陀螺仪，无磁力计） |
| 左腕 | `SL` | 1440×960 | 30fps | 8 Mbps | H.265 视频 + IMU 码带 |
| 右腕 | `JHHSW` | 1440×960 | 30fps | 8 Mbps | H.265 视频 + IMU 码带 |

**IMU 码带说明**：每个相机的图像**顶部有一条横向码带**，把 IMU 数据（ICM42688 加速度计 + 陀螺仪）编码成像素行嵌进视频帧。程序从每帧图像把这个码带解码出来，得到与视频逐帧对齐的 IMU 数据。三路相机各带一路码带，共 3 路 IMU。**板端实测**：头部 SLZH 双目只输出加速度 + 陀螺仪样本，无磁力计（AK09940）。
...
> 注意：头部 SLZH 双目（`1bcf:2d56`）与腕部（`1bcf:2d52`）PID 不同，但同属 1bcf 系列，多路并发时仍需实测确认 UVC 流数量是否受限。
