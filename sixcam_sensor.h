...
protected:
    void setup() override {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            fprintf(stderr, "[%s] DBG: handle=%p\n", ch.name, ch.tstc_handle);

            ch.dev_fd = open(devs[i]->Device_Path, O_RDWR | O_NONBLOCK);
            if (ch.dev_fd < 0) {
                fprintf(stderr, "[%s] open %s failed: %s\n",
                        ch.name, devs[i]->Device_Path, strerror(errno));
                return;
            }
            fprintf(stderr, "[%s] DBG: dev_fd=%d\n", ch.name, ch.dev_fd);
        ...
        // ── 6. ★ 逐通道完全串行: stream线程 → STREAM_STATUS ──
        //    SixCamSensor 在同一线程中串行处理两个通道,
        //    避免 TSTC SDK 对同 VID/PID 设备的并发冲突.
        for (int i = 0; i < 2; i++) {
            auto& ch = ch_[i];
