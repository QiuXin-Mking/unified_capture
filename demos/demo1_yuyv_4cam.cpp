// demo1_yuyv_4cam.cpp — 四目通道 YUYV → Y8
//
//   用法: ./demo1_yuyv_4cam <device> <num_frames> <output.y8>
//   示例: ./demo1_yuyv_4cam /dev/video6 30 fourcam_yuyv.y8
//
...
int main(int argc, char* argv[]) {
    if (argc != 4) {
        ...
        fprintf(stderr, "示例: %s /dev/video6 30 fourcam_yuyv.y8\n", argv[0]);
        return 1;
    }
    ...
    int stride = cap.bytesperline();  // YUYV 行跨度（可能 > width*2）
    ...
    // ── 预分配 Y8 行缓冲区 ─────────────────────────────────────
    std::vector<uint8_t> y8_row(width);

    // ── 采集循环 ────────────────────────────────────────────────
    fprintf(stderr, "[demo1] 四目 YUYV → Y8, %dx%d, %d 帧\n", width, height, num_frames);
    ...
    fprintf(stderr, "[demo1] 完成: %d 帧 %.2f 秒, %.1f fps, %.1f MB/s, 输出 %s (%zu bytes)\n",
