// demo3_mjpeg_4cam.cpp — 四目通道 MJPEG → Y8（当前生产管线路径）
//
//   用法: ./demo3_mjpeg_4cam <device> <num_frames> <output.y8>
//   示例: ./demo3_mjpeg_4cam /dev/video6 30 fourcam_mjpeg.y8
//
...
int main(int argc, char* argv[]) {
    if (argc != 4) {
        ...
        fprintf(stderr, "示例: %s /dev/video6 30 fourcam_mjpeg.y8\n", argv[0]);
        return 1;
    }
    ...
    fprintf(stderr, "[demo3] 四目 MJPEG → Y8, %dx%d, %d 帧\n", width, height, num_frames);
    ...
    while (captured < num_frames) {
        ...
        // ── Y 平面 → Y8 ───────────────────────────────────────
        int y_stride = jpeg_width;  // turbojpeg 返回紧密排列的平面
    ...
    fprintf(stderr, "[demo3] 完成: %d 帧 %.2f 秒, %.1f fps, %.1f MB/s, 输出 %s (%zu bytes)\n",
