#!/usr/bin/env python3
"""
calc_sync.py — 多相机曝光同步分析

从 IMU JSONL 提取每帧 exp_start_us, 按相机类型分组对比。
用法: python3 calc_sync.py <session_dir>
      python3 calc_sync.py user@host:<session_path>
"""
...
def main():
    ...
    if not target:
        print("用法: python3 calc_sync.py <session_dir>")
        print("      python3 calc_sync.py user@host:<session_path>")
        sys.exit(1)
    ...
    for name, starts in sorted(cameras.items()):
        ...
        print(f"  {name:16s} {len(starts):5d}帧  {duration_s:.1f}s  "
              f"{avg_fps:.1f}fps  exp_start: {starts[0]} → {starts[-1]}  "
    ...
    for group_name, group in [("六目相机 (JHH)", jhh), ("腕部相机 (Wrist)", wrist)]:
        ...
        print_sep(f"{group_name} — 两两曝光起点差值分布")
        ...
        for i, na in enumerate(names):
            for j, nb in enumerate(names):
                ...
                within_10ms = sum(1 for d in deltas if d <= 10_000)
                rate = within_10ms / len(deltas) * 100

                print(f"  {na:16s} ↔ {nb:16s}  "
                      f"配对 {len(deltas)} 个, "
                      f"≤10ms: {within_10ms}/{len(deltas)} ({rate:.1f}%)  "
