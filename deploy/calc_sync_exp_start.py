#!/usr/bin/env python3
"""
calc_sync_exp_start.py — 多相机曝光起点同步分析

从 IMU JSONL 提取每帧 exp_start_us, 按相机类型分组及跨组对比。
用法: python3 calc_sync_exp_start.py <session_dir>
      python3 calc_sync_exp_start.py user@host:<session_path>
"""
...
def main():
    ...
    if not target:
        print("用法: python3 calc_sync_exp_start.py <session_dir>")
        print("      python3 calc_sync_exp_start.py user@host:<session_path>")
        sys.exit(1)
    ...
    if cross_pairs:
        print_sep("六目 (JHH) ↔ 腕部 (Wrist) — 曝光起点差值分布")
